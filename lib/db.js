...
// 特定の本をブリックリストから削除
// INPUT: uid(String), bid(String)
export async function removeItemFromBookList(uid, bid) {
  const user = await firestore.collection('users').doc(uid).get()
  ...
  // 削除を完了していないなら、非ISBNのリストも確認する
  if (bookListWithoutISBN.length > 0 && !done) {
    for (const item of bookListWithoutISBN) {
      if (item.bid === bid) {
        // 該当する item の index を取得
        const index = bookListWithoutISBN.indexOf(item)

        if (index > -1) {
          // 削除
          bookListWithoutISBN.splice(index, 1)
        }
      }
    }
  }
...
// 特定の本の進捗管理を自動に戻す
// INPUT: uid(String), bid(String)
export async function turnOnAutoProgress(uid, bid) {
  const user = await firestore.collection('users').doc(uid).get()
  const userInfo = user.data()
  ...
  if (bookList.length > 0) {
    for (const item of bookList) {
      if (item.bid === bid) {
        // autoProgress を true に変更
        item.autoProgress = true
